...
def db_add_report(user_id, username, description, photo_path, lat, lon, email_status):
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute(
            "INSERT INTO reports(user_id, username, description, photo_path, lat, lon, created_at, email_status) "
            "VALUES(?,?,?,?,?,?,?,?)",
            (user_id, username, description, photo_path, lat, lon, datetime.utcnow().isoformat(), email_status)
        )
        con.commit()

def db_last_reports(user_id, limit=10):
    with closing(sqlite3.connect(DB_PATH)) as con:
        cur = con.execute(
            "SELECT id, description, lat, lon, created_at, email_status "
...
# ---------- MAIL ----------
async def send_mail(subject: str, html_body: str, to_email: str, attachment_path: Path|None):
    # Если SMTP не настроен — пропустим отправку (удобно для MVP)
...
async def my_reports(m: Message):
    ...
    if not rows:
        return await m.answer("У вас пока нет заявок.")
    lines = ["<b>Последние заявки</b>"]
    for rid, desc, lat, lon, created, status in rows:
        loc = f" (📍{lat:.5f},{lon:.5f})" if (lat is not None and lon is not None) else ""
...
async def got_photo(m: Message, state: FSMContext):
    ...
    path = DATA_DIR / f"{m.from_user.id}_{file.file_unique_id}.jpg"
    await bot.download_file(file.file_path, destination=path)

    d = (await state.get_data())["draft"]
    draft = ReportDraft(**d); draft.photo_path = str(path)
...
async def need_photo(m: Message):
    await m.answer("Пожалуйста, отправьте фото.")

@dp.message(ReportFlow.wait_description, F.text)
async def got_desc(m: Message, state: FSMContext):
    d = (await state.get_data())["draft"]
    draft = ReportDraft(**d); draft.description = m.text.strip()[:300]
    await state.update_data(draft=draft.__dict__)
...
async def got_loc(m: Message, state: FSMContext):
    d = (await state.get_data())["draft"]
    draft = ReportDraft(**d); loc: Location = m.location
...
async def skip_loc(m: Message, state: FSMContext):
    await finalize_and_send(m, state)
...
async def finalize_and_send(m: Message, state: FSMContext):
    ...
    place = f"\nКоординаты: {draft.lat:.5f}, {draft.lon:.5f}" if (draft.lat is not None and draft.lon is not None) else ""
    html = (
    ...
    db_add_report(
        ...
        draft.description or "", draft.photo_path or "",
        draft.lat, draft.lon, status
    )

    await state.clear()
    await m.answer(
        "Спасибо! Обращение отправлено: <b>" + ("успешно" if status == "sent" else status) + "</b>.",
