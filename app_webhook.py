...
async def my_reports(m: Message):
    # простая выборка последних 10
    with closing(sqlite3.connect(DB_PATH)) as con:
        cur = con.execute("SELECT id, description, lat, lon, created_at, email_status FROM reports WHERE user_id=? ORDER BY id DESC LIMIT 10", (m.from_user.id,))
        rows = cur.fetchall()
    if not rows:
...
async def finalize_and_send(m: Message, state: FSMContext):
    ...
    with closing(sqlite3.connect(DB_PATH)) as con:
        con.execute("INSERT INTO reports(user_id, username, description, photo_path, lat, lon, created_at, email_status) VALUES(?,?,?,?,?,?,?,?)",
                    (m.from_user.id, m.from_user.username, draft.description or '', draft.photo_path or '', draft.lat, draft.lon, datetime.utcnow().isoformat(), status))
        con.commit()
...
async def on_startup(app: web.Application):
    ...
    if PUBLIC_URL:
        ...
        url = PUBLIC_URL.rstrip('/') + f'/webhook/{BOT_TOKEN}'
        try:
            await bot(SetWebhook(url=url, drop_pending_updates=True))
